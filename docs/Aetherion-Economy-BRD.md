...
| 2026-08-23 | THE MAIL SAGA, SOLVED IN TWO ACTS. Act one (throttle): errand-holders exempted from the activity throttle - measured before: 994 mailbox verdicts, zero completed walks; after: 613 bots physically within 40yd of a mailbox (73 under 10yd). Act two (the guid): DB-spawned gameobjects carry a map-generated lowguid, so the trip target guid built from (entry, spawnId) resolved NOTHING - every GO-target arrival fell into the MoveRandomNear fallback, bots orbiting the mailbox they could not see. Fix: resolve the live object via the map's spawnId store at leg-selection time. Also repairs E7 focus trips (same construction). Diagnosis was pure telemetry: verdict census x distance-band query, no log spelunking | Deployed to production same hour; watchers armed for first mail_collect in realm history (3,314 letters pending). Delivery infra parallel-tracked: repo pushed to GitHub as nlucansk, v0.1.0 release + changelog workflow, public-org mirror workflow proven, CI validate 4/4 GREEN (pnpm 11 allowBuilds unblocked the frontend job) |

| 2026-08-23 | THE POST OFFICE OPENS. First mailbox collection in realm history at 19:26:09 (Marzumzast) - 126 seconds after the spawnId-fix binary booted, the 73 bots already standing at mailboxes fired within three seconds of each other. 518 collections by 516 distinct bots in the first ten minutes, 744 within twenty; verdict backlog draining 1470 -> 1043; realm money-mail down to 10 letters (400g) from the full backlog. Tracked windfall veteran Ugsi collected at 19:26:15 | E5.2 VALIDATED - the last open soak item. The loop is closed end to end: earn -> list -> sell -> proceeds mailed -> COLLECTED -> fund the next need. Every epic of the core program is now observed working in production |

| 2026-08-23 | E7.5 NEEDS-DRIVEN GATHERING built, adversarially verified, deployed, and observed - all in one evening. Herb/vein chest spawns indexed per (map, skill) from Lock.dbc at startup; a gatherer earns a trip only with a live reason (own reagent shortfall, or an unfunded need to farm toward) and only to nodes inside its skill tier; arrival hands the node to the existing reactive loot chain (which owns skill/tool/distance checks and the cast). Verification caught a chain-killer pre-deploy: the vendor branch's config-only else-if consumed the verdict chain for every bot, which would have made gather dead code - its bag-pressure test now lives in the branch condition | 25 min after deploy: 587 gather verdicts in the census and the first node arrivals - Negorm (80, mining 400) at a Saronite Deposit in Northrend (req 400, tier-perfect) and Editia (7, herbalism 37) at a starting-zone herb. The sixth errand kind is live; census now spans all of them (694 mailbox / 587 gather / 182 ah / 91 trainer / 46 focus / 5 vendor) |

| 2026-08-23 | E7.5 HARVEST VALIDATED: Editia's herbalism ticked 37 -> 40 within the first soak hour - skill-ups only come from a successful gather cast on a node, so the full chain is proven: needs gate -> verdict -> tier-matched routing -> arrival -> reactive loot chain -> harvest. Arrivals 3 -> 11 across 45 min (gather is deliberately the lowest-priority errand; pace rises as the 694-deep mailbox queue drains). Mail collections crossed 2,300 by 1,400+ distinct bots in the first two hours | E7.5 moves from deployed to OBSERVED WORKING. Remaining unvalidated soak item realm-wide: first bank withdrawal (E6.3b) - organically rare, needs a bot whose banked mats match a current recipe shortfall |

| 2026-08-24 | E9 observability leap, beyond the BRD's own list: new MARKET tab (bot-to-bot trades feed with both parties named, market depth by item class, hot items, biggest sales, richest ten, wealth histogram, supply sparkline, sinks-vs-faucets with an honesty caption for unlogged sinks) and ECON tab extensions (hourly activity small-multiples, live six-verdict errand census, mail health, needs-funding bars, craft output, nodes visited by name, profession ladder). Four agent-built endpoint+panel pairs, every query validated against the live realm pre-ship. CI pivoted per operator call: hosted full-compile canary cancelled and made manual-only; a 49-test pure-logic pytest suite for the bridge now runs in validate (whole pipeline green in under a minute) | The market's texture is finally visible: 904 listings/109 sellers/43 sales a day of steady small commerce; Velriale revealed as realm postmaster (1,922 letters, 384 runs); Northrend node gathering on the board (Saronite, Talandra's Rose) |

| 2026-08-24 | PERSONA DUTY VALIDATED, two hours of soak: party formation 5,143/hr against ~170/hr during the errand-monopoly collapse (and ~1,250/hr on a normal day), zone travel 8,046/hr, world loot 1,416/hr - the social simulation fully recovered while ~1,355 errand verdicts stand deferred-not-dropped and econ throughput holds at 20k events/hr. Census: 1,699 farmers / 754 adventurers / 35 merchants / 12 warlords - the farmer share (68%) runs hot because the factory gives most bots gather+craft pairs; the hash splits are the tuning knob if a leaner farmer class is wanted | The economy and the society now share the clock instead of fighting over it. Same day: raid formats shipped (25-man seats, per-destination difficulty, heroic attempts behind seeded gear floors - first 10-man raids to Naxx/VoA/OS observed with explicit difficulty logging, dissolve safety valve fired in the wild), and the Observatory shipped its full redesign (OVERVIEW tab, four palettes, ten screens rebuilt) |

| 2026-08-24 | RAID FORMATS VALIDATED IN THE WILD, evening session: first heroic dungeon trip (Honklefuk's party, The Forge of Souls heroic - visible on the operator's board within the minute) and FIRST 25-MAN RAID IN REALM HISTORY (Treld leads 25 to Naxxramas at true 25-player difficulty), raised by the muster's realm-wide call-up after 367 capped Horde answered. Getting here took three design iterations driven by live measurement: (1) the flat 180 gear floor against the fifteen-slot DF average failed 45% of the capped fleet and five-of-five passing made heroics sub-percent - entry heroics re-seeded to the era's 160; (2) the muster's one-map seating could never fill against a realm-wide ungrouped bench of ~23 - it now seats realm-wide and summon-gathers; (3) per the operator's call, hard gear gates became SCALED APPETITE - the party average against the floor sets the heroic chance (full at floor, zero at floor minus twenty), normal raids soft-gate at floor minus fifteen, quests and attunements stay hard, and the portal cliff turned off | The gear ladder is now a gradient, not a wall: undergeared parties occasionally take brave swings, all-green parties stay home, and nobody is a one-member veto. Heroic runs wear an SVG skull on the PVE kanban and overview boards |
